...
|---|---|---|---|---|---|---|---|
| IDX-DATA-002 | DATA | Execute auditable Yahoo EOD backfill + official IDX identity/tradability ingestion for adversarial cases; emit provenance and unresolved gaps, no modelling | Luna xhigh / xhigh | `1ebfe625` | isolated DATA worktree | current data foundation | READY |
| IDX-VAL-002 | VALIDATION | Independently audit announcement completeness evidence, official snapshot reconciliation, corporate-action/raw-price verification, and adversarial DATA GATE outputs | Luna xhigh / xhigh | `1ebfe625` | read-only or isolated VALIDATION worktree | IDX-DATA-002 artifacts | BLOCKED_ON_DATA |
...
Do not create modelling workers merely because DATA workers are idle. The current
critical path is DATA -> VALIDATION -> full gate -> independent certification.
