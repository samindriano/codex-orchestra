    ...
    *,
    ...
    if actual_worker_count is not None:
        if isinstance(actual_worker_count, bool) or actual_worker_count < 0:
            raise LauncherError("actual_worker_count must be a non-negative integer")
        metadata.update(
            {
                "actual_worker_count": actual_worker_count,
                "worker_presence_quality": "EXACT",
                "worker_presence_source": "LAUNCHER_EXPLICIT",
            }
        )
...
def run(
    ...
    *,
    ...
    environment[METADATA_ENV] = json.dumps(
        launcher_metadata(mode, user_args, actual_worker_count=actual_worker_count),
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=True,
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not values or values[0] not in _CONFIG:
        print("usage: codex_benchmark_launcher.py {standard|fast} [--actual-worker-count N] [-- <codex arguments...>]", file=sys.stderr)
        return 2
    mode = values.pop(0)
    actual_worker_count: int | None = None
    ...
    while index < len(values) and values[index] != "--":
        value = values[index]
        if value == "--actual-worker-count":
            if index + 1 >= len(values):
                print("BENCHMARK_LAUNCHER_ERROR=--actual-worker-count requires N", file=sys.stderr)
                return 2
            try:
                actual_worker_count = int(values[index + 1])
            except ValueError:
                print("BENCHMARK_LAUNCHER_ERROR=--actual-worker-count requires an integer", file=sys.stderr)
                return 2
            index += 2
            continue
        if value.startswith("--actual-worker-count="):
            try:
                actual_worker_count = int(value.split("=", 1)[1])
            except ValueError:
                print("BENCHMARK_LAUNCHER_ERROR=--actual-worker-count requires an integer", file=sys.stderr)
                return 2
            index += 1
        ...
        index += 1
    values = launcher_args + values[index:]
    if values[:1] == ["--"]:
        values.pop(0)
    try:
        return run(mode, values, actual_worker_count=actual_worker_count)
