...
class BenchmarkLauncherTests(unittest.TestCase):
    ...
    def test_metadata_can_attest_explicit_zero_workers(self) -> None:
        metadata = launcher.launcher_metadata("standard", actual_worker_count=0)
        self.assertEqual(metadata["actual_worker_count"], 0)
        self.assertEqual(metadata["worker_presence_quality"], "EXACT")
        self.assertEqual(metadata["worker_presence_source"], "LAUNCHER_EXPLICIT")
