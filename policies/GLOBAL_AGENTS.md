...
Astra Medium is a `MANUAL_EXPERIMENTAL` profile requiring
`EXPLICIT_USER_OPT_IN`. It is never auto-selected or auto-escalated for difficulty,
conflict, review, HEAVY work, ambiguity, or disagreement.
...
`global-astra` profile selects `ASTRA_ROOT` and `$astra-decision-orchestrator`.
