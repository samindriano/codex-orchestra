...
The legacy unprofiled root default is GPT-5.6 Luna XHigh; configured worker roles
are GPT-6 Luna Max. Profiles are below project and CLI configuration in native
precedence. `global-gpt6-luna` pins a GPT-6 Luna Max root; `global-gpt6-sol` pins a
GPT-6 Sol High root; both keep GPT-6 Luna Max workers. The `global-astra-low` and
...
and its workers remain GPT-6 Luna Max; effort never escalates automatically. A project
...
No automatic model-conditional AGENTS syntax is used. A newly created config gets
the legacy Luna XHigh root default; updates preserve any existing root model
selection. Use explicit profiles for reproducible GPT-6 root routing. The desktop
